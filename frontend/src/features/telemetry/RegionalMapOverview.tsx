...
export function RegionalMapOverview() {
  ...
  const reloadDashboard = useCallback(async () => {
    ...
    try {
      await Promise.all([loadGeo(), loadWeather(), loadAqi()]);
    } finally {
  ...
  const mlMapSub = "Districts Map and Weather";
  ...
  return (
    ...
    >
      ...
      <div className={`grid gap-6 lg:grid-cols-5 transition-opacity duration-300 ${isRefreshingMap ? "opacity-40" : ""}`}>
        {}
