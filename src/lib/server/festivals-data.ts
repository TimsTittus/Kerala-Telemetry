...
async function fetchCalendarific(year: number): Promise<FestivalItem[]> {
  ...
  if (!key) return [];
  const apiBase = process.env.CALENDARIFIC_API_BASE!;
  ...
  try {
    ...
    console.warn("Calendarific fetch failed:", e);
    return [];
  }
}

async function fetchNagerDate(year: number): Promise<FestivalItem[]> {
  const apiBase = process.env.NAGER_DATE_BASE!;
  ...
  try {
    ...
    const text = await res.text();
    if (!text.trim()) return [];
    const holidays = JSON.parse(text) as {
      date: string;
      localName: string;
      name: string;
      types: string[];
    ...
    console.warn("NagerDate fetch failed:", e);
    return [];
  }
}
...
export async function getFestivalsPayload(): Promise<FestivalsPayload> {
  const today = new Date().toISOString().slice(0, 10);
  try {
    ...
    return { items, source, error: null };
  } catch (e) {
